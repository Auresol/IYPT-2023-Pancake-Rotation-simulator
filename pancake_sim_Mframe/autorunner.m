function [resultarr,outomega] = autorunner(n)
    ...
    plodisplayseperate = 100;

    omega_condition = 1;
    velocity_condition = 1;
    ...
    while alltime < limtime
        ...
        for idx = 1:n
            ...
            [Fi,fi] = contactforce_wall(ballarr(idx),container_radius,container_velocity,container_contact_velocity,isincontactwitlwall,delta_t);
            ...
            Fi = Fi + [0,-1,0] * grav_const;
            %ballarr(idx).velocity = ballarr(idx).velocity + container_velocity;
    
            coriolisforce = 2 * ballarr(idx).mass * cross(2*pi*container_frequency,ballarr(idx).velocity) * 0.05;
            coriolisforce(3) = 0;
        ...
        for iter = 1:n
            ...
            if magnitude(ballarr(iter).angular_velocity - 2*pi*container_frequency) > omega_condition && magnitude(ballarr(iter).velocity - container_omega * ballarr(iter).position) > velocity_condition
                set(graphics(iter),'Facecolor',[0,1,0]);
            else
                set(graphics(iter),'Facecolor',[0,0,1]);
            end
        end
        ...
        if pauseiteration == renderhertz && isrender == true
            ...
            if isrendervector == true
                ...
                for circle = 1:n 
                    ...
                    velocityvector(circle) = line(position(1) + [0, ballarr(circle).velocity(1)/velomag/100], position(2) + [0,ballarr(circle).velocity(2)/velomag/100], 'color', 'black');
